...
function add_subscriber($cnx, $table_email, $list_id, $add_addr, $table_email_deleted)
{
    ...
    if ($hash == '') {
        ...
        if ($black_listed == '') {
            ...
            if (
                $cnx->query("INSERT INTO " . $table_email . " (`email`, `list_id`, `hash`) 
...
function createNewsletter(
    ...
    if (
        !$cnx->query("INSERT INTO $table_listsconfig (`list_id` , `newsletter_name` , `from_addr` , 
...
function delete_subscriber($cnx, $table_email, $list_id, $del_addr, $table_email_deleted, $motif)
{
    ...
    if ($CPTID['CPTID'] > 0) {
        if (
            !$cnx->query("INSERT IGNORE INTO $table_email_deleted
                              SELECT * 
                                  FROM $table_email 
            ...
            $cnx->query("UPDATE $table_email_deleted 
                             SET error='Y', type='" . ($motif != '' ? $motif : 'unsub') . "'
                                 WHERE email=" . escape_string($cnx, $del_addr) . "
                                     AND list_id=" . $list_id);
            if (
                !$cnx->query("DELETE FROM $table_email 
                    WHERE list_id = " . $list_id . " 
...
function delete_subscriber_tmp($cnx, $table_email, $list_id, $del_addr, $table_email_deleted, $motif)
{
    ...
    if ($CPTID['CPTID'] > 0) {
        if (
            !$cnx->query("INSERT IGNORE INTO $table_email_deleted (email, list_id, hash)
...
function force_subscriber($cnx, $table_email_tmp, $list_id, $del_addr, $table_email, $hash)
{
    ...
    if ($CPTID['CPTID'] > 0) {
        if (
            !$cnx->query("INSERT IGNORE INTO $table_email (list_id,email,hash) 
            VALUES (" . escape_string($cnx, $list_id) . "," . escape_string($cnx, $del_addr) . "," . escape_string($cnx, $hash) . ")")
        ) {
            return false;
        } else {
            if (
                !$cnx->query("DELETE FROM $table_email_tmp 
                    WHERE list_id = '$list_id' 
                        AND email='$del_addr'")
            ) {
                return false;
...
function deleteNewsletter($cnx, $table_list, $table_archives, $table_email, $table_temp, $table_send, $table_tracking, $table_autosave, $list_id)
{
    ...
    if (
        !$cnx->query("DELETE $table_tracking,$table_send 
                        FROM $table_tracking 
...
function getAddress($cnx, $table_email, $list_id, $msg_id, $begin = '', $limit = '')
{
    $limite = (isset($limit)) ? " LIMIT 0,$limit" : "";
    return $Addr = $cnx->query("SELECT id,email,hash 
                                    FROM $table_email 
...
function getArchivesSelectList($cnx, $table_archives, $list_id, $msg_id = '', $form_name = 'archive_form2')
{
    $row = $cnx->query("SELECT id, date, subject FROM $table_archives WHERE list_id='$list_id' ORDER BY date DESC")->fetchAll(PDO::FETCH_ASSOC);
    if (count($row) == 0) {
        return -1;
...
function getSenders($cnx, $table_senders, $sender = '')
{
    ...
    if (count($row) == 0) {
        ...
        foreach ($row as $x) {
            ...
            if ($x['email'] == $sender && $sender != '')
                $liste_senders .= ' selected';
...
function UpdateEmailError(
    ...
    if ($hash != '') {
        if (
            $cnx->query("INSERT IGNORE INTO " . $table_email_deleted . " (id,email,list_id,hash,error,status,type,categorie,short_desc,long_desc,campaign_id)
